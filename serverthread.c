...
// use to communicate with each other whether they are using a certain file.
...
int create_listen_socket(uint16_t portNum) {
  ...
  int listenfd = socket(AF_UNSPEC, SOCK_STREAM, 0);
  if (listenfd < 0) {
...
int main(int argc, char *argv[]) 
{
  ...
  //listenfd = create_listen_socket(port1);
  // assign IP, PORT
